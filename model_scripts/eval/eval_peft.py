...
def main():
    ...
    # 1. Define the 8-bit config (MATCHING YOUR FT SCRIPT)
    ...
    # 2. Pass it to the 'quantization_config' argument
    ...
    # 3. Load the processor FROM THE ARTIFACT
    print(f"Loading processor from base model: {BASE_MODEL_NAME}...")
    processor = AutoProcessor.from_pretrained(BASE_MODEL_NAME)

    import os
    if os.path.exists(os.path.join(model_source, "tokenizer.json")):
        print(f"Overwriting tokenizer with artifact version...")
        from transformers import AutoTokenizer
        processor.tokenizer = AutoTokenizer.from_pretrained(model_source)
    ...
    zs_types = ['context', 'mcq', 'pure']
    
    # If zs_type is None or frozen_vision, it loads the adapter (fine-tuned eval).
    # If zs_type is 'pure', etc., it skips loading the adapter (base model eval).
    if(zs_type not in zs_types):
        # 4. Apply the LoRA adapter (the PEFT weights)
        print(f"Applying PEFT adapter from {model_source}...")
        model = PeftModel.from_pretrained(model, model_source)
    else:
        print(f"Zero-shot mode detected ('{zs_type}'). Skipping PEFT adapter.")
    ...
    def create_dataloader(dataset, zs_type, batch_size=8):
        ...
        return DataLoader(dataset, batch_size=batch_size, collate_fn=collator, num_workers=8, persistent_workers=True)
